...
def get_distinct():
    """
        Get the number of service types.
    """
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT distinct service FROM conn LIMIT 10;"
        )
...
def get_count(from_time=None, to_time=None, specific=None):
    """
        Get the count of each service type.
    """
    ...
    if specific:
        ...
        arr = ", ".join(specific)
        specific_sql = "service IN ({}) ".format(arr)
    else:
        specific_sql = ""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT service, count(*) as cnt FROM conn " + \
            condition_sql + \
            "GROUP BY service " + \
            "ORDER BY cnt DESC " + \
            "LIMIT 10;"
...
def get_stat_by(page_size=10, page_id=None, from_time=None, to_time=None):
    """
        Get the count of 
        alert_num, proto_num, src_ip_num, src_port_num, dest_ip_num, dest_port_num
        for each proto type.
    """
    if from_time and to_time:
...
def get_specific_stat_by(service, stat, page_size=10, page_id=None, from_time=None, to_time=None):
    """
        Get the count and timestamp of distinct stat:stat 
        for records with service type of param:service
    """
    if stat == "src_ip":
        stat = "orig_h"
    elif stat == "src_port":
    ...
    """
    with connection.cursor() as cursor:
        count = cursor.execute(sql, service)
        if page_id and page_size:
            cursor.scroll((page_id-1) * page_size, mode='absolute')
            data = cursor.fetchmany(page_size)
        else:
