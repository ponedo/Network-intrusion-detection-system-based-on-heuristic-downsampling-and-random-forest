...
def get_distinct():
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT distinct orig_h FROM conn LIMIT 10;"
        )
...
def get_count(from_time=None, to_time=None, specific=None):
    ...
    if specific:
        ...
        arr = ", ".join(specific)
        specific_sql = "orig_h IN ({}) ".format(arr)
    else:
        specific_sql = ""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT orig_h, count(*) as cnt FROM conn " + \
            condition_sql + \
            "GROUP BY orig_h " + \
            "ORDER BY cnt DESC " + \
            "LIMIT 10;"
...
def get_specific_stat_by(orig_h, stat, page_size=10, page_id=None, from_time=None, to_time=None):
    """
    ...
    """
    with connection.cursor() as cursor:
        count = cursor.execute(sql, orig_h)
        if page_id and page_size:
            cursor.scroll((page_id-1) * page_size, mode='absolute')
            data = cursor.fetchmany(page_size)
        else:
