...
def get_distinct():
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT distinct alert FROM conn LIMIT 10;"
        )
...
def get_count(from_time=None, to_time=None, specific=None):
    """
        Get the count of each alert type.
    """
    ...
    if specific:
        ...
        arr = ", ".join(specific)
        specific_sql = "alert IN ({}) ".format(arr)
    else:
        specific_sql = ""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT alert, count(*) as cnt FROM conn " + \
            condition_sql + \
            "GROUP BY alert " + \
            "ORDER BY cnt DESC " + \
            "LIMIT 10;"
...
def get_stat_by(page_size=10, page_id=None, from_time=None, to_time=None):
    """
        Get the count of 
        proto_num, src_ip_num, src_port_num, dest_ip_num, dest_port_num, service_num
        for each alert type.
    """
    if from_time and to_time:
...
def get_specific_stat_by(alert, stat, page_size=10, page_id=None, from_time=None, to_time=None):
    """
        Get the count and timestamp of distinct stat:stat 
        for records with alert type of param:alert
    """
    if stat == "src_ip":
        stat = "orig_h"
    elif stat == "src_port":
    ...
    """
    with connection.cursor() as cursor:
        count = cursor.execute(sql, alert)
        if page_id and page_size:
            cursor.scroll((page_id-1) * page_size, mode='absolute')
            data = cursor.fetchmany(page_size)
        else:
