...
def get_distinct():
    """
        Get the number of distinct orig_p.
    """
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT distinct orig_p FROM conn LIMIT 10;"
        )
...
def get_count(from_time=None, to_time=None, specific=None):
    """
        Get the count of each distinct orig_p.
    """
    ...
    if specific:
        ...
        arr = ", ".join(specific)
        specific_sql = "orig_p IN ({}) ".format(arr)
    else:
        specific_sql = ""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT orig_p, count(*) as cnt FROM conn " + \
            condition_sql + \
            "GROUP BY orig_p " + \
            "ORDER BY cnt DESC " + \
            "LIMIT 10;"
...
def get_stat_by(page_size=10, page_id=None, from_time=None, to_time=None):
    """
        Get the count of 
        alert_num, proto_num, src_ip_num, dest_ip_num, dest_port_num, service_num
        for each proto type.
    """
    if from_time and to_time:
...
def get_specific_stat_by(orig_p, stat, page_size=10, page_id=None, from_time=None, to_time=None):
    """
        Get the count and timestamp of distinct stat:stat 
        for records with orig_p type of param:orig_p
    """
    if stat == "src_ip":
        stat = "orig_h"
    elif stat == "src_port":
    ...
    """
    with connection.cursor() as cursor:
        count = cursor.execute(sql, orig_p)
        if page_id and page_size:
            cursor.scroll((page_id-1) * page_size, mode='absolute')
            data = cursor.fetchmany(page_size)
        else:
